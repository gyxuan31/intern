...
class ModelPredictiveControl(object):
    ...
    def __init__(self,A,B,C,f,v,W3,W4,x0,desiredControlTrajectoryTotal):
        ...
        self.O, self.M, self.gainMatrix = self.formLiftedMatrices()  #42
    ...
    def propagateDynamics(self,controlInput,state): # controlInput = u(k), state = x(k)

        xkp1=np.zeros(shape=(self.n,1)) # x(k+1) = Ax(k) + Bu(k)
        yk=np.zeros(shape=(self.r,1)) # y(k) = Cx(k)
    ...
    def computeControlInputs(self): #main function
